...
export async function GET(request){
  ...
  return NextResponse.json({hasKey:!!user?.heygenApiKey, masked:mask(user?.heygenApiKey)});
}
export async function POST(request){
  ...
  if(!test.ok) return NextResponse.json({error:"HeyGen key failed validation", detail:test.detail?.slice?.(0,200)||`HTTP ${test.status}`},{status:400});
