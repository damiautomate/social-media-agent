...
import { getUser, getDraft, getBrandConfig } from "@/lib/content-bank.js";
...
export async function POST(request) {
  ...
  // Check avatar/voice selection exists
  const cfg = await getBrandConfig(auth.userId);
  const av = cfg?.videoStyle?.avatar;
  if (!av?.avatarId || !av?.voiceId) {
    return NextResponse.json(
      { error: "No avatar/voice selected. Settings → HeyGen → pick an avatar and voice." },
      { status: 400 },
    );
  }

  // Don't pile up jobs on the same draft
  if (draft.avatarVideoStatus === "generating") {
    return NextResponse.json(
      { error: "Avatar video generation already in progress for this draft" },
      { status: 429 },
    );
  }
