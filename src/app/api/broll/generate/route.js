...
import { getUser, getDraft, createPendingJob } from "@/lib/content-bank.js";
...
export async function POST(request) {
  ...
  const safeClipCount = safeMode === "storyboard" ? Math.min(Math.max(Number(clipCount) || 3, 2), 5) : 1;
  ...
  if (draft.brollStatus === "generating") return NextResponse.json({ error: "B-roll generation already in progress for this draft" }, { status: 429 });

  const jobId = await createPendingJob(auth.userId, { type: "broll", draftId, mode: safeMode, clipCount: safeClipCount });
  return NextResponse.json({ jobId, status: "queued", mode: safeMode, clipCount: safeClipCount });
}
