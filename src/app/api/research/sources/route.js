...
function validateSource(s) {
  ...
  if (s.type === "reddit" && !s.config.subreddit) return "Reddit source needs config.subreddit";
  if (s.type === "rss" && !s.config.url) return "RSS source needs config.url";
  if (s.type === "youtube" && !s.config.channelId) return "YouTube source needs config.channelId";
  if (s.type === "hackernews" && (s.config.minScore == null || isNaN(Number(s.config.minScore)))) return "HN source needs numeric config.minScore";
  return null;
}
...
function validateResearchPatch(patch) {
  ...
  if (patch.targetIdeasPerRun != null) {
    ...
    if (!Number.isInteger(n) || n < 1 || n > 50) return "targetIdeasPerRun must be an integer 1-50";
  }
  if (patch.dedupeWindowDays != null) {
    const n = Number(patch.dedupeWindowDays);
    if (!Number.isInteger(n) || n < 1 || n > 90) return "dedupeWindowDays must be an integer 1-90";
  }
  if (patch.scoringWeights) {
    const w = patch.scoringWeights;
    const total = ["relevance", "novelty", "voiceFit", "urgency"].map((k) => Number(w[k]) || 0).reduce((a, b) => a + b, 0);
  ...
  if (patch.sources != null) {
    ...
    for (const s of patch.sources) { const err = validateSource(s); if (err) return err; }
  }
  return null;
}
...
export async function GET(request) {
  ...
  const research = (config && config.research && Object.keys(config.research).length) ? config.research : DEFAULT_BRAND_TEMPLATE.research;
...
export async function PUT(request) {
  ...
  const base = (existing.research && Object.keys(existing.research).length) ? existing.research : DEFAULT_BRAND_TEMPLATE.research;
